...
func TestFFTAudit_EnergyScalingConsistency(t *testing.T) {
    ...
    for tr := 0; tr < trials; tr++ {
        ...
        for i := 0; i < N; i++ { coeffs[i] = rng.NormFloat64() }
        // To Eval via negacyclic embedding
        cf := ps.NewFieldElemBig(N, prec)
        for i := 0; i < N; i++ { cf.Coeffs[i].Real.SetFloat64(coeffs[i]) }
        ev := ps.FloatToEvalNegacyclic(cf, prec)
        // Compute energies
        var eCoeff, eEval float64
        for i := 0; i < N; i++ { eCoeff += coeffs[i]*coeffs[i] }
        for i := 0; i < N; i++ {
            r, _ := ev.Coeffs[i].Real.Float64(); im, _ := ev.Coeffs[i].Imag.Float64()
            eEval += r*r + im*im
        }
        if eCoeff == 0 { t.Fatalf("zero coeff energy") }
        ratios = append(ratios, eEval/eCoeff)
    }
    // Check ratios are consistent within a small relative tolerance
    mean := 0.0
    for _, r := range ratios { mean += r }
    mean /= float64(len(ratios))
    var dev float64
    for _, r := range ratios { d := r - mean; dev += d * d }
