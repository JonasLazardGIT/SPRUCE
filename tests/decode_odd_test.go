...
func TestDecodeOdd_ParityAndWorstFlip(t *testing.T) {
    ...
    if err != nil { t.Fatalf("DecodeOdd: %v", err) }
    if parity(out) != 1 { t.Fatalf("expected odd parity, got even: %v", out) }
    // RoundToEven baseline would round all to 0; worst is index 0; flip upward to 1
    if out[0] != 1 { t.Fatalf("index 0 must be flipped to 1; got %v", out) }
    for i := 1; i < len(out); i++ {
        if out[i] != 0 { t.Fatalf("unexpected change at %d: %v", i, out) }
    }
    ...
    if err != nil { t.Fatalf("DecodeOdd: %v", err) }
    if parity(out2) != 1 { t.Fatalf("expected odd parity, got even: %v", out2) }
    ...
    if err != nil { t.Fatalf("DecodeOdd: %v", err) }
    if parity(out3) != 1 { t.Fatalf("expected odd parity, got even: %v", out3) }
...
func TestDecodeOdd_RandomSanity(t *testing.T) {
    in := make([]float64, 64)
    for i := range in { in[i] = math.Sin(float64(i))*3.25 + 0.3*math.Cos(float64(7*i)) }
    out, err := ntru.DecodeOdd(in)
    if err != nil { t.Fatalf("DecodeOdd: %v", err) }
    if parity(out) != 1 { t.Fatalf("expected odd parity, got even: %v", out) }
}
